...
*   `resolve(value)`: Call this when the job is done *successfully*, with the result `value`.
*   `reject(error)`: Call this if there's an *error*, with the error object.
...
*   `resolve` or `reject` should only be called *once*.  Subsequent calls are ignored.
...
*   **`Promise.race(iterable)`:** Waits for the *first* promise to settle. Returns its result/error.
```javascript {all}
    Promise.race([
      new Promise(resolve => setTimeout(() => resolve(1), 1000)),
...
*   **`Promise.any(iterable)`:** Waits for the *first* promise to fulfill (resolve). Returns its result. If all reject, rejects with `AggregateError`.
...
As for unhandled rejections, JavaScript only fires the `unhandledrejection` event *after* the microtask queue is empty and there's still a rejected promise without a `.catch`.
